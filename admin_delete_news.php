...
// Ambil data admin untuk Navbar (mirip admin_add_edit_gallery.php)
...
$news_data = [ // Inisialisasi $news_data
    'news_title' => '',
    'news_content' => '',
    'news_image_path' => null, // Akan menyimpan nama file
    'is_published' => 1 // Default terbit untuk item baru
];
$error_messages = [];
$success_message = "";

define('NEWS_UPLOAD_DIR', 'uploads/news_images/'); // Direktori upload berita

// Jika ada ID berita di URL (mode edit), ambil data dari database
if ($news_id) {
    $query = "SELECT news_title, news_content, news_image_path, is_published FROM news WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($result && $fetched_data = mysqli_fetch_assoc($result)) {
            $news_data = $fetched_data; // $news_data['news_image_path'] akan berisi nama file
        } else {
            $error_messages[] = "Berita dengan ID tersebut tidak ditemukan.";
            // Set $news_id ke null agar form kembali ke mode tambah jika ID tidak valid,
            // atau bisa juga redirect ke halaman manage berita dengan pesan error.
            // Untuk halaman add/edit terpisah, menampilkan error di form lebih umum.
            $news_id = null;
            $page_title = "Tambah Berita Baru"; // Reset page title
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $posted_title = trim($_POST['news_title'] ?? '');
    $posted_content = trim($_POST['news_content'] ?? '');
    $posted_is_published = isset($_POST['is_published']) ? 1 : 0;
    // current_image_filename berisi nama file gambar yang sudah ada (jika edit)
    $current_image_filename = $_POST['current_image_filename'] ?? $news_data['news_image_path'] ?? null;
    
    // Isi $news_data dengan nilai yang di-POST untuk repopulasi form jika ada error
    $news_data['news_title'] = $posted_title;
    $news_data['news_content'] = $posted_content;
    $news_data['is_published'] = $posted_is_published;
    // $news_data['news_image_path'] akan diupdate jika ada upload baru atau tetap $current_image_filename

    $new_image_filename = $current_image_filename; // Default ke gambar yang sudah ada (atau null)
    ...
    if (isset($_FILES['news_image_file']) && $_FILES['news_image_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (empty($error_messages)) { // Lanjutkan hanya jika direktori OK
            ...
            if ($_FILES['news_image_file']['size'] > 5 * 1024 * 1024) { // 5MB
            ...
            if (empty($error_messages)) {
                if (move_uploaded_file($_FILES['news_image_file']['tmp_name'], $target_file_path)) {
                    $new_image_filename = $unique_filename; // Update dengan nama file baru yang unik
                    // Hapus gambar lama jika ini mode edit, ada gambar lama, dan gambar baru diupload & berbeda
        ...
        // Jika ini mode tambah baru DAN tidak ada gambar diupload (dan tidak ada gambar lama)
        $error_messages[] = "Anda harus mengunggah gambar untuk berita baru.";
    }
    // Update $news_data['news_image_path'] dengan nama file yang relevan (baru atau lama)
    $news_data['news_image_path'] = $new_image_filename;

    // Jika tidak ada error validasi, lakukan INSERT atau UPDATE
    if (empty($error_messages)) {
        if ($news_id) { // Mode EDIT
            $sql = "UPDATE news SET news_title = ?, news_content = ?, news_image_path = ?, is_published = ?, updated_at = NOW() WHERE id = ?";
            ...
            if ($stmt_db) {
                ...
                if (mysqli_stmt_execute($stmt_db)) {
                    ...
                    // $news_data sudah diupdate di atas, jadi form akan menampilkan data terbaru
                } else {
            ...
            $sql = "INSERT INTO news (news_title, news_content, news_image_path, is_published, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())";
            $stmt_db = mysqli_prepare($conn, $sql);
            if ($stmt_db) {
                mysqli_stmt_bind_param($stmt_db, 'sssi', $posted_title, $posted_content, $new_image_filename, $posted_is_published);
                if (mysqli_stmt_execute($stmt_db)) {
                    $new_inserted_id = mysqli_insert_id($conn); // Dapatkan ID berita yang baru ditambahkan
                    $_SESSION['manage_news_success_message'] = "Berita baru berhasil ditambahkan."; // Pesan untuk halaman manage
                    header("Location: admin_manage_news.php?highlight_id=" . $new_inserted_id); // Redirect ke halaman manage
                    exit;
                } else {
    ...
    // Jika ada error validasi, $news_data sudah diisi ulang dengan data POST dan $new_image_filename
}
...
<body class="admin-authenticated">
    ...
    <div class="container container-form mt-4">
        ...
        <form action="admin_add_edit_news.php<?= ($news_id ? '?id='.$news_id : ''); ?>" method="POST" enctype="multipart/form-data">
            <div class="mb-3">
                <label for="news_title_input" class="form-label">Judul Berita <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="news_title_input" name="news_title" value="<?= htmlspecialchars($news_data['news_title']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="news_content_input" class="form-label">Isi Berita <span class="text-danger">*</span></label>
                <textarea class="form-control" id="news_content_input" name="news_content" rows="10" required><?= htmlspecialchars($news_data['news_content']); ?></textarea>
            </div>
            <div class="mb-3">
                ...
                <?php if (!empty($news_data['news_image_path'])): ?>
                    ...
                    <?php if (file_exists(NEWS_UPLOAD_DIR . $news_data['news_image_path'])): ?>
                        ...
                        <img src="<?= htmlspecialchars(NEWS_UPLOAD_DIR . $news_data['news_image_path']); ?>?t=<?= time(); // Cache buster ?>" alt="Gambar Berita Saat Ini" class="img-thumbnail img-thumbnail-preview">
            ...
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="is_published_input" name="is_published" value="1" <?= ($news_data['is_published'] == 1) ? 'checked' : ''; ?>>
    ...
    <script>
        // Pencegahan akses halaman admin dari cache (BACK button) setelah logout
        // Lebih efektif ditangani di sisi server (cek session di setiap halaman admin)
        ...
        //     window.location.href = 'logout.php'; // Atau ke halaman login
        // }
        ...
        window.addEventListener('load', function () {
            if (!document.body.classList.contains('admin-authenticated')) {
                // console.warn('Admin class not found, potential issue or handled by PHP.');
                // window.location.href = 'login.php'; // Uncomment jika ingin enforce via JS
            }
        });
    </script>
</body>
