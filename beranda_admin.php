...
$db_name = 'smap9589_sma_ibnu_aqil';         // Nama database Anda
$db_user = 'smap9589';                       // User MySQL yang Anda berikan
$db_pass = 'Xxz82UMAcDYy11';                 // Password yang Anda berikan
...
if (isset($_GET['action']) && $_GET['action'] === 'download_excel') {
    if (file_exists('config.php')) {
        ...
        exit('File konfigurasi database tidak ditemukan.');
    }
    ...
    if (isset($_GET['search_term']) && $_GET['search_term'] != '') {
        ...
        $where_clause_export .= " AND (name LIKE '%$search_term_safe%')";
    }
    ...
    if (!function_exists('formatPhoneNumberForExcel')) {
        function formatPhoneNumberForExcel($phone) {
            ...
            return !empty($cleaned_phone) ? '="' . $cleaned_phone . '"' : '';
        }
    }
    ...
    $column_headers_inline = [
        'ID Pendaftar', 'Nama Lengkap', 'Tanggal Lahir (DOB)', 'No. Telepon', 'Alamat Lengkap', 'Status Pendaftaran', 'Tanggal Daftar',
        'Tempat Lahir', 'Jenis Kelamin', 'Agama Siswa', 'Hobi Siswa', 'Cita-cita', 'Motivasi',
        'Nama Ayah', 'No. Telepon Ayah', 'Pekerjaan Ayah', 'Email Ayah', 'Penghasilan Ayah', 'Tempat Lahir Ayah', 'Tanggal Lahir Ayah', 'Agama Ayah',
        'Nama Ibu', 'No. Telepon Ibu', 'Pekerjaan Ibu', 'Email Ibu', 'Penghasilan Ibu', 'Tempat Lahir Ibu', 'Tanggal Lahir Ibu', 'Agama Ibu',
        'Nama Wali', 'No. Telepon Wali', 'Hubungan Wali', 'Email Wali', 'Penghasilan Wali', 'Tempat Lahir Wali', 'Tanggal Lahir Wali', 'Agama Wali',
        'Path File KK Siswa', 'Path File Akta Lahir', 'Path File Nilai Rapor',
        'Path File KTP Ayah', 'Path File KTP Ibu', 'Path File KTP Wali', 'Path File KK Wali'
    ];
    ...
    if (mysqli_num_rows($result_export_inline) > 0) {
        while ($row_export = mysqli_fetch_assoc($result_export_inline)) {
            $csv_row_inline = [
                $row_export['id'] ?? '', $row_export['name'] ?? '', 
                formatDateForExport($row_export['dob'] ?? ''),
                formatPhoneNumberForExcel($row_export['phone'] ?? ''),
                $row_export['address'] ?? '', $row_export['status'] ?? '',
                formatDateTimeForExport($row_export['created_at'] ?? ''),
                $row_export['birthplace'] ?? '', $row_export['gender'] ?? '', $row_export['religion_child'] ?? '',
                $row_export['student_hobby'] ?? '', $row_export['goal'] ?? '', $row_export['motivation'] ?? '',
                $row_export['father_name'] ?? '',
                formatPhoneNumberForExcel($row_export['father_phone'] ?? ''),
                $row_export['father_job'] ?? '', $row_export['father_email'] ?? '', $row_export['father_income'] ?? '',
                $row_export['father_birthplace'] ?? '', formatDateForExport($row_export['father_dob'] ?? ''), $row_export['religion_father'] ?? '',
                $row_export['mother_name'] ?? '',
                formatPhoneNumberForExcel($row_export['mother_phone'] ?? ''),
                $row_export['mother_job'] ?? '', $row_export['mother_email'] ?? '', $row_export['mother_income'] ?? '',
                $row_export['mother_birthplace'] ?? '', formatDateForExport($row_export['mother_dob'] ?? ''), $row_export['religion_mother'] ?? '',
                $row_export['guardian_name'] ?? '',
                formatPhoneNumberForExcel($row_export['guardian_phone'] ?? ''),
                $row_export['guardian_relation'] ?? '', $row_export['guardian_email'] ?? '', $row_export['guardian_income'] ?? '',
                $row_export['guardian_birthplace'] ?? '', formatDateForExport($row_export['guardian_dob'] ?? ''), $row_export['guardian_religion'] ?? '',
                $row_export['kk_file'] ?? '', $row_export['akta_lahir'] ?? '', $row_export['nilai_rapor'] ?? '',
                $row_export['ktp_father'] ?? '', $row_export['ktp_mother'] ?? '',
                $row_export['ktp_guardian'] ?? '', $row_export['kk_guardian'] ?? ''
            ];
...
// Pastikan koneksi $conn berhasil
...
// Asumsi admin_id atau admin disimpan di session saat login
$admin_id = $_SESSION['admin_id'] ?? $_SESSION['admin'] ?? null; 
$admin_name = "Admin"; // Default
$admin_photo_url = 'public/asset/user.png'; // Default path jika tidak ada foto spesifik

if ($admin_id) {
    if (isset($_SESSION['admin_name'])) $admin_name = htmlspecialchars($_SESSION['admin_name']);
    
    // Coba ambil URL foto dari session terlebih dahulu
    ...
    // Atau jika hanya filename yang disimpan dan perlu digabungkan dengan path
    elseif (isset($_SESSION['admin_photo_filename']) && !empty($_SESSION['admin_photo_filename'])) {
        ...
        if (file_exists($potential_admin_photo_path)) { // Periksa keberadaan file di server
...
$counts = [
    ...
    'incomplete_students' => 0, 'rejected_students' => 0, 
...
$queries_for_counts = [
    ...
    'facilities' => "SELECT COUNT(*) AS count_val FROM facilities", // Asumsi tabel facilities
    'news' => "SELECT COUNT(*) AS count_val FROM news", // Asumsi tabel news
    'gallery' => "SELECT COUNT(*) AS count_val FROM gallery" // Asumsi tabel gallery
];
...
if (!empty($search_term_val)) {
    ...
    $where_clause_display_students .= " AND (name LIKE '%$search_term_safe%')";
}

$students_list_for_dashboard = [];
$query_students_display = "SELECT id, name, phone, created_at, status, father_name FROM students $where_clause_display_students ORDER BY created_at DESC LIMIT 10"; // Tambahkan email untuk pencarian
...
<head>
    ...
    <style>
        ...
        .dashboard-card { width: 220px; padding: 20px; text-align: center; border-radius: 8px; font-weight: bold; color: white; box-shadow: 0 4px 8px rgba(0,0,0,0.1); transition: transform 0.2s ease; text-decoration: none; }
        .dashboard-card:hover { transform: translateY(-5px); color:white; } /* Pastikan teks tetap putih saat hover */
        .dashboard-card h3 { margin-top: 8px; font-size: 2.2rem; line-height:1; } /* Ukuran H3 diperbesar */
        .dashboard-card .card-text {font-size: 0.9rem; margin-bottom: 5px; opacity:0.9;}
        .dashboard-card .icon { font-size: 2.5rem; margin-bottom: 10px; display:block; } /* Ikon lebih besar & block */

        .card-total-pendaftar { background: linear-gradient(135deg, #0d6efd, #0a58ca); }
        .card-terverifikasi { background: linear-gradient(135deg, #198754, #146c43); }
        .card-pending { background: linear-gradient(135deg, #fd7e14, #e85a00); }
        .card-ditolak-incomplete { background: linear-gradient(135deg, #dc3545, #b02a37); }
        .card-fasilitas { background: linear-gradient(135deg, #6f42c1, #59359a); }
        .card-berita { background: linear-gradient(135deg, #20c997, #15a37a); }
        .card-galeri { background: linear-gradient(135deg, #d63384, #a31f60); }
        
        .filter-search-container { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 20px; align-items: center; justify-content: flex-end; padding: 15px; background-color:#f8f9fa; border-radius: 8px; border: 1px solid #dee2e6;}
        .action-buttons .btn { margin-right: 5px; margin-bottom: 5px; padding: 0.25rem 0.5rem; font-size:0.8rem; } /* Tombol aksi lebih kecil */
         td.action-buttons { white-space: nowrap; } /* Agar tombol tidak wrap jika memungkinkan */
        th, td { min-width: 80px; vertical-align: middle; font-size:0.875rem; }
        th.no-column, td.no-column { min-width: 40px; width: 40px; text-align: center;}
        th.aksi-column, td.aksi-column { min-width: 150px; text-align:center; } /* Disesuaikan agar 3 tombol muat */
    </style>
</head>
<body class="admin-authenticated">
    ...
    <div class="container mt-4">
        ...
        <div class="dashboard-options">
            <a href="admin_manage_students.php?status_filter=all" class="dashboard-card card-total-pendaftar text-decoration-none">
                <i class="bi bi-people-fill icon"></i><span class="card-text">Total Pendaftar</span><h3><?= $counts['total_students'] ?? 0; ?></h3>
            </a>
            <a href="admin_manage_students.php?status_filter=verified" class="dashboard-card card-terverifikasi text-decoration-none">
                <i class="bi bi-person-check-fill icon"></i><span class="card-text">Terverifikasi</span><h3><?= $counts['verified_students'] ?? 0; ?></h3>
            </a>
            <a href="admin_manage_students.php?status_filter=pending" class="dashboard-card card-pending text-decoration-none">
                <i class="bi bi-hourglass-split icon"></i><span class="card-text">Pending</span><h3><?= $counts['pending_students'] ?? 0; ?></h3>
            </a>
            <a href="admin_manage_students.php?status_filter=incomplete_or_rejected" class="dashboard-card card-ditolak-incomplete text-decoration-none"> 
                <i class="bi bi-person-x-fill icon"></i><span class="card-text">Ditolak/Belum Lengkap</span><h3><?= ($counts['incomplete_students'] ?? 0) + ($counts['rejected_students'] ?? 0); ?></h3>
            </a>
            <a href="admin_manage_facilities.php" class="dashboard-card card-fasilitas text-decoration-none">
                <i class="bi bi-buildings-fill icon"></i><span class="card-text">Total Fasilitas</span><h3><?= $counts['facilities'] ?? 0; ?></h3>
            </a>
            <a href="admin_manage_news.php" class="dashboard-card card-berita text-decoration-none">
                <i class="bi bi-journal-text icon"></i><span class="card-text">Total Berita</span><h3><?= $counts['news'] ?? 0; ?></h3>
            </a>
            <a href="admin_manage_gallery.php" class="dashboard-card card-galeri text-decoration-none">
                <i class="bi bi-card-image icon"></i><span class="card-text">Total Galeri</span><h3><?= $counts['gallery'] ?? 0; ?></h3>
            </a>
        </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover table-striped align-middle">
                            ...
                            <?php
                            // PHP ini akan terus merender tabel pada page load pertama atau submit manual
                            if (isset($students_table_error)): ?>
                                <tr><td colspan="7" class="text-center text-danger py-3"><em><?php echo $students_table_error; ?></em></td></tr>
        ...
        if (!empty($students_list_for_dashboard)) {
            foreach ($students_list_for_dashboard as $row_modal_hapus_data){ 
                ...
                if (file_exists($template_modal_hapus_path_pendaftar)) {
                    include $template_modal_hapus_path_pendaftar;  
                }
            }
        }
        // Bebaskan memori hasil query students jika belum
        if (isset($result_students_display_query) && $result_students_display_query instanceof mysqli_result) { 
            mysqli_free_result($result_students_display_query); 
        }
        ?>
    </div> <div class="modal fade" id="studentDetailModal" tabindex="-1" aria-labelledby="studentDetailModalLabel" aria-hidden="true">
    ...
    function downloadPDF() {
        ...
        const elementsToHideForPDF = elementToPrint.querySelectorAll('.action-buttons, .filter-search-container, .mb-3.text-center.text-md-start .btn');
        ...
        const opt = {
            ...
            html2canvas:  { scale: 2, logging: true, useCORS: true, scrollY: -window.scrollY },
            jsPDF:        { unit: 'mm', format: 'a4', orientation: 'landscape' }
        };

        html2pdf().from(elementToPrint).set(opt).save().then(function () {
            elementsToHideForPDF.forEach(el => el.style.display = '');
    ...
    // --- KODE JAVASCRIPT FINAL UNTUK LIVE SEARCH DAN LIHAT DETAIL ---
    document.addEventListener('DOMContentLoaded', function() {
        // !!! PENTING: JALUR INI SUDAH DIKONFIRMASI BENAR UNTUK SETUP ANDA DI CPANEL !!!
        const BASE_UPLOAD_URL = '/uploads/'; 
        
        var detailModalBody = document.getElementById('studentDetailModalBody'); // Target modal body detail
        var liveSearchInput = document.getElementById('liveSearchInput'); // Input pencarian
        var filterStatusSelect = document.getElementById('filterStatusSelect'); // Dropdown filter status

        // Fungsi helper untuk membuat link file yang benar
        function getFileLink(filePath) {
            ...
            // Asumsi folder admin tidak di root, misalnya /admin/
            // Sesuaikan BASE_UPLOAD_URL jika path di database sudah lengkap atau admin ada di root
            if (filePath.toLowerCase().startsWith('uploads/')) { // Jika DB sudah menyimpan 'uploads/namafile.jpg'
                 // Cek apakah admin ada di subfolder (misal /admin/) atau di root
                 if (window.location.pathname.toLowerCase().includes('/admin/')) { 
                     // Jika admin ada di /admin/, maka uploads ada di ../uploads/
                     fullPublicPath = '../' + filePath; 
                 } else {
                     // Jika admin ada di root, uploads juga di root
                     fullPublicPath = filePath; 
                 }
            } else if (filePath.startsWith('http://') || filePath.startsWith('https://') || filePath.startsWith('/')) {
                 fullPublicPath = filePath;
            } else { // Jika hanya nama file (misal 'namafile.jpg')
                 fullPublicPath = BASE_UPLOAD_URL + fileNameOnly.trim();
            }
            
            // Perbaikan untuk memastikan tag <a> benar
            return `<a href="${fullPublicPath}" target="_blank" class="btn btn-sm btn-outline-info"><i class="bi bi-file-earmark-fill me-1"></i> Lihat File</a>`;
        }

        // --- FUNGSI UNTUK TRIGGER PENCARIAN OTOMATIS (LIVE SEARCH) ---
        // Ini akan mensubmit form secara otomatis setelah user selesai mengetik
        let debounceTimer;
        function triggerSearch() {
            clearTimeout(debounceTimer);
            debounceTimer = setTimeout(() => {
                // Submit form agar PHP melakukan filter dan pencarian
                // Ini akan me-reload halaman, tapi dengan parameter pencarian baru
                document.querySelector('.filter-search-container form').submit();
            }, 300); // Debounce 300ms (menunggu 0.3 detik setelah ketikan terakhir)
        }

        // --- Event Listeners untuk Live Search ---
        // Pastikan ID 'liveSearchInput' dan 'filterStatusSelect' ada di HTML Anda
        if (liveSearchInput) {
            liveSearchInput.addEventListener('input', triggerSearch);
        }
        if (filterStatusSelect) {
            filterStatusSelect.addEventListener('change', triggerSearch);
        }

        // --- KODE UNTUK MODAL LIHAT DETAIL (SUDAH DIPERBAIKI DAN DIOPTIMASI) ---
        document.querySelectorAll('.view-detail-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if(detailModalBody) {
                    detailModalBody.innerHTML = '<p class="text-center my-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><br>Memuat detail pendaftar...</p>';
                }
                if(studentDetailModal) studentDetailModal.show();

                // Lakukan permintaan AJAX ke file PHP yang benar (sesuaikan nama file jika Anda pakai nama lain)
                // Berdasarkan percakapan kita, Anda menggunakan 'Workspace_student_details.php'
                fetch('Workspace_student_details.php?id=' + studentId)
                    .then(response => {
                        if (!response.ok) {
                            return response.text().then(text => { 
                                throw new Error('Network response error: ' + response.status + ' ' + response.statusText + '. Response: ' + text.substring(0, 300)) 
                            });
                        }
